...
import sys
sys.path.append('../')
from utils import alt_tqa_evaluate, flattened_idx_to_layer_head, layer_head_to_flattened_idx, get_interventions_dict, get_top_heads, get_separated_activations, get_com_directions, save_probes, probe_top_1_head_with_top_k_neurons
...
def main(): 
    ...
    for i in range(args.num_fold):
        ...
        top_heads, probes, all_k_accs_np = probe_top_1_head_with_top_k_neurons(train_set_idxs, val_set_idxs, separated_head_wise_activations, separated_labels, num_layers, num_heads, args.seed, args.num_heads, head_dim, args.use_random_dir, 'mean_diff')
        np.save(f'../features/{args.model_name}_{args.dataset_name}_probe_acc_with_top_k_neurons_mean_diff_fold_{i}.npy', all_k_accs_np)
        top_heads, probes, all_k_accs_np = probe_top_1_head_with_top_k_neurons(train_set_idxs, val_set_idxs, separated_head_wise_activations, separated_labels, num_layers, num_heads, args.seed, args.num_heads, head_dim, args.use_random_dir, 'probe_coef')
        np.save(f'../features/{args.model_name}_{args.dataset_name}_probe_acc_with_top_k_neurons_probe_coef_fold_{i}.npy', all_k_accs_np)
        top_heads, probes, all_k_accs_np = probe_top_1_head_with_top_k_neurons(train_set_idxs, val_set_idxs, separated_head_wise_activations, separated_labels, num_layers, num_heads, args.seed, args.num_heads, head_dim, args.use_random_dir, 'random')
        np.save(f'../features/{args.model_name}_{args.dataset_name}_probe_acc_with_top_k_neurons_random_fold_{i}.npy', all_k_accs_np)
